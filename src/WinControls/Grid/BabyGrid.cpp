...
void DisplayColumn(HWND hWnd,int SI,int c,int offset,HFONT hfont,HFONT hcolumnheadingfont)
{
	...
	else
	{
		if(!GetNextColWithWidth(SI,c,1))
		{
			...
			trect.top+=BGHS[SI].titleheight;
			FillRect(gdc,&trect,(HBRUSH)(COLOR_WINDOW+1));
		}
	}
	 ...
	 while(r<=BGHS[SI].bottomvisiblerow)
	 {
		 ...
		 else
		 {
			  ...
			  if((r==BGHS[SI].cursorrow) && BGHS[SI].GRIDHASFOCUS)
				  DrawFocusRect(gdc,&rect);
...
ATOM RegisterGridClass(HINSTANCE hInstance)
{
	...
	COLORREF BGColor,textcolor,highlightcolor,highlighttextcolor,cursorcolor,gridlinecolor;
	COLORREF protectcolor,unprotectcolor;
	DWORD color_distance,r,g,b;

	BGColor=GetSysColor(COLOR_BACKGROUND);
	textcolor=GetSysColor(COLOR_WINDOWTEXT);
	highlighttextcolor=GetSysColor(COLOR_HIGHLIGHTTEXT);
	cursorcolor=RGB(~GetRValue(BGColor),~GetGValue(BGColor),~GetBValue(BGColor));
	gridlinecolor=GetSysColor(COLOR_ACTIVEBORDER);
	highlightcolor=GetSysColor(COLOR_HIGHLIGHT);
	protectcolor=BGColor;
	unprotectcolor=BGColor;
	r=GetRValue(BGColor)-GetRValue(textcolor);
	g=GetGValue(BGColor)-GetGValue(textcolor);
	b=GetBValue(BGColor)-GetBValue(textcolor);
	color_distance=r*r+g*g+b*b;
	if(color_distance<(0x30*0x30*3)){
		protectcolor=cursorcolor;
		unprotectcolor=cursorcolor;
	}
   ...
   for(int j = 0 ; j < MAX_GRIDS ; j++)
   {
		...
		BGHS[j].cursorcolor = cursorcolor;
		BGHS[j].protectcolor = protectcolor; 
		BGHS[j].unprotectcolor = unprotectcolor;
		BGHS[j].highlightcolor = highlightcolor; 
		BGHS[j].gridlinecolor = gridlinecolor;
        BGHS[j].highlighttextcolor = highlighttextcolor;
		BGHS[j].textcolor = textcolor;
